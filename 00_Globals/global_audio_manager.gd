...
func play_music( _audio : AudioStream, _volume : float = 0.0 ) -> void:
	...
	play_and_fade_in( current_player, _volume )
...
func play_and_fade_in( player : AudioStreamPlayer, _final_volume : float = 0 ) -> void:
	player.play( 0 )
	var tween : Tween = create_tween()
	tween.tween_property( player, "volume_db", _final_volume, music_fade_duration )
	pass
