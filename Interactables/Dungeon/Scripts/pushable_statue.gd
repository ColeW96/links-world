class_name PusahableStatue extends RigidBody2D

@export var push_speed : float = 30.0
@export var persistent : bool = false
@export var persistent_location : Vector2 = Vector2.ZERO
@export var target_location_size : Vector2 = Vector2( 2, 2 )
...
func _physics_process(_delta: float) -> void:
	...
	if persistent:
		var x_is_on : bool = abs( position.x - persistent_location.x ) < 11 + target_location_size.x
		var y_is_on : bool = abs( position.y - persistent_location.y ) < 3 + target_location_size.y
